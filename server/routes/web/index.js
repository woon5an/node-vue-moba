module.exports = app => {
    ...
    //导入新闻数据
    router.get('/news/init', async (req, res) =>{           //将新闻录入进数据库
        const parent = await Category.findOne({
            name: '新闻分类'
        })
        const cats = await Category.find().where({
            parent: parent                              //只找新闻分类下的子分类
        }).lean()           //lean方法表示取纯粹的json，js对象，数组
        ...
        const newsList = newsTitles.map(title => {
            const randomCats = cats.slice(0).sort((a,b) => Math.random() - 0.5)    //表示从cats中复制一份数组出来，再用sort去排序
            return {
                categories: randomCats.splice(0,2),
                title: title
            }
        })
        await Article.deleteMany({})          //先把数据库里文章模块的数据清空  ‘{}’表示以任意的条件去清空
    ...
    router.get('/news/list', async (req, res) => {
        ...
        //     name: '新闻分类'
        // }).populate({
        //     path: 'children',    //把新闻分类的子分类调出来 
        //     populate: {
        //         path: 'newsList'    //再把newsList关联
        //     }
        // }).lean()
        const parent = await Category.findOne({
            name: '新闻分类'
        })
        //MongoDB的聚合查询,在一次查询里面用类似于mysql的关联或者更强大的方式同时执行好几条查询最终得到想要的结果
        const cats = await Category.aggregate([
            { $match: { parent: parent._id} },   //通过match过滤一些数据
            { 
                $lookup: {                       //关联查询
    ...
    router.get('/heroes/list', async (req, res) => {
        ...
        const cats = await Category.aggregate([
            ...
            { 
                $lookup: {                       //关联查询
                    from: 'heroes',//schema的集合名默认情况下是schema名字的小写复数形式
                    localField: '_id',
                    foreignField: 'categories',
                    as: 'heroList'
                }
